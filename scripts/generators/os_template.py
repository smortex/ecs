...
def save_composable_template(ecs_version, component_names, out_dir, mapping_settings_file, template_settings_file):
    ...
    filename = join(out_dir, "opensearch/composable/template.json")
...
def generate_legacy_template_version(
    ...
    ecs_helpers.make_dirs(join(out_dir, 'opensearch', "legacy"))
    template: Dict = template_settings(ecs_version, mappings_section, template_settings_file, is_legacy=True)

    filename: str = join(out_dir, "opensearch/legacy/template.json")
    save_json(filename, template)
...
def opensearch_field_type(type):
    """Given an Elasticsearch type, return the corresponding OpenSearch type"""
    if type == "flattened":
        return "flat_object"
    return type
...
def entry_for(field: Field) -> Dict:
    field_entry: Dict = {'type': opensearch_field_type(field['type'])}
    try:
        ...
        if field['type'] == 'keyword' or field['type'] == 'flattened':
            ecs_helpers.dict_copy_existing_keys(field, field_entry, ['ignore_above', 'value'])
